...
//
//  TransformProtocol+getErrorFromResponseAndBody.swift
//  SmokeHTTPMiddleware
//
...
extension TransformProtocol {
    func getErrorFromResponseAndBody<ErrorType: Error & Decodable>(errorTypeHTTPHeaderValue: String,
        ...
        guard let bodyAsDictionary = serializedBody as? [String: Any] else {
            throw SdkError<ErrorType>.client(.deserializationFailed(DeserializationError.notExpectedType), response)
        }
